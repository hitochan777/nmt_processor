...
class LexicalDictionary:
    ...
    def filter_vocab(self, dic):
        assert self.vocab is not None, "You need to set vocab beforehand"
        filtered_vocab = []
        for word in dic:
    ...
    def set_vocab(self, vocab):
        assert self.finalized, "You need to finalize before setting vocabulary"
        assert isinstance(vocab, set)
        self.vocab = vocab

    def get_translations(self, cond, only_in_vocab = True):
        """
    ...
    @classmethod
    def create_lexical_table(cls, filename, topn=1):
